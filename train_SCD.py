...
from lib.models import BGSNet as Net
...
uwl = AutomaticWeightedLoss(5)

def main():        
    net = Net(64, num_classes=RS.num_classes, pretrained_path='/HOME/code/BGSNet_SCD/pretrained/pvt_v2_b2.pth').cuda()
    ...
    optimizer1 = optim.AdamW(uwl.parameters(), lr=0.00001)

    train(train_loader, net, criterion, optimizer, scheduler, val_loader, optimizer1)
    writer.close()
    print('Training finished.')

def train(train_loader, net, criterion, optimizer, scheduler, val_loader, optimizer1):
    bestaccT=0
    bestFscdV=0.0
    bestloss=1.0
    ...
    while True:
        ...
        for i, data in enumerate(train_loader):
            ...
            loss_edge = cretion1(out_bd, semantic_bd)
...
def validate(val_loader, net, criterion, curr_epoch):
    ...
    for vi, data in enumerate(val_loader):
        ...
        with torch.no_grad():
            ...
            # loss_A = criterion(outputs_A, labels_A)*0.5
            # loss_B = criterion(outputs_B, labels_B)*0.5
            # loss = loss_A + loss_B
