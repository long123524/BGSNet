...
def rand_flip_MCD(img1, img2, label1, label2, lable_c):
    r = random.random()
    # showIMG(img.transpose((1, 2, 0)))
    if r < 0.25:
        return img1, img2, label1, label2, lable_c
    elif r < 0.5:
        return np.flip(img1, axis=0).copy(), np.flip(img2, axis=0).copy(), np.flip(label1, axis=0).copy(), np.flip(label2, axis=0).copy(), np.flip(lable_c, axis=0).copy()
    elif r < 0.75:
        return np.flip(img1, axis=1).copy(), np.flip(img2, axis=1).copy(), np.flip(label1, axis=1).copy(), np.flip(label2, axis=1).copy(), np.flip(lable_c, axis=1).copy()
    else:
        return img1[::-1, ::-1, :].copy(), img2[::-1, ::-1, :].copy(), label1[::-1, ::-1].copy(), label2[::-1, ::-1].copy(), lable_c[::-1, ::-1].copy()
...
def rand_rot90_MCD(img1, img2, label1, label2, label3):
    r = random.random()
    # showIMG(img.transpose((1, 2, 0)))
    if r < 0.5:
        return img1, img2, label1, label2, label3
    else:
        return np.rot90(img1).copy(), np.rot90(img2).copy(), np.rot90(label1).copy(), np.rot90(label2).copy(), np.rot90(label3).copy()
...
def rand_rot90_flip_MCD(img1, img2, label1, label2, label3):
    img1, img2, label1, label2, label3 = rand_rot90_MCD(img1, img2, label1, label2, label3)
    return rand_flip_MCD(img1, img2, label1, label2, label3)
